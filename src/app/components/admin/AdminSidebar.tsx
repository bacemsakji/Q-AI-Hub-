...
export function AdminSidebar({ activeSection, onNavigate }: AdminSidebarProps) {
    return (
        <aside className="fixed left-0 top-0 z-40 flex h-screen w-64 flex-col bg-background/95 backdrop-blur-xl border-r border-border">
            {/* Logo */}
            <motion.div
                            ...
                            >
                                <button
                                    ...
                                    className={`group relative flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-300 ${isActive
                                        ? 'bg-[#00E5FF]/15 text-cyan-600 dark:text-[#00E5FF] shadow-[0_0_15px_rgba(0,229,255,0.1)]'
                                ...
                                >
                                    ...
                                    {isActive && (
                                        <motion.div
                                            ...
                                            className="absolute inset-0 rounded-xl border border-primary/20 shadow-soft"
                                            transition={{ type: 'spring', stiffness: 300, damping: 30 }}
                                        />
                                    )}
                                    <item.icon
                                        className={`relative z-10 h-[18px] w-[18px] transition-all duration-300 ${isActive ? 'text-cyan-500 dark:text-[#00E5FF]' : 'text-muted-foreground group-hover:text-foreground/60'
                                            }`}
                                    />
            ...
            <motion.div
                ...
                className="mx-3 my-4 rounded-xl bg-card/40 p-3 border border-border backdrop-blur"
            >
                <div className="flex items-center gap-3">
                    <div className="relative">
                        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-primary/30 to-accent/30 text-xs font-bold text-primary ring-2 ring-primary/20">
                            AD
                        </div>
                        {/* Online indicator */}
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="truncate text-[11px] text-muted-foreground">admin@qaihub.com</p>
                    </div>
                </div>
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-border">
                    <ThemeToggle />
                    <button className="flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground transition-all hover:bg-foreground/5 hover:text-foreground/80">
