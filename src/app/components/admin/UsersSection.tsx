...
function UserStatusBadge({ status }: { status: string }) {
    const s: Record<string, string> = {
        Active: 'bg-[#00FFC2]/15 text-[#00FFC2] border-[#00FFC2]/20', Pending: 'bg-[#FFB800]/15 text-[#FFB800] border-[#FFB800]/20', Suspended: 'bg-[#FF4757]/15 text-[#FF4757] border-[#FF4757]/20',
    };
    return <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-semibold ${s[status] || 'bg-white/10 text-white/70 border-white/10'}`}>{status}</span>;
}
...
function AppStatusBadge({ status }: { status: string }) {
    ...
    return <span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ${s[status] || 'bg-white/10 text-white/70'}`}>{status}</span>;
}
...
function UserProfilePanel({ user, onClose }: { user: AdminUser; onClose: () => void }) {
    return (
        <motion.div
            ...
            className="fixed inset-y-0 right-0 z-50 flex w-[500px] flex-col border-l border-white/[0.06] bg-[#070B14]/95 backdrop-blur-xl shadow-2xl shadow-black/50"
        >
            ...
            <div className="flex-1 overflow-y-auto">
                <div className="border-b border-white/[0.06] bg-gradient-to-br from-[#00E5FF]/[0.04] to-[#7B2FFF]/[0.02] p-6">
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-2"><h4 className="text-xl font-bold text-white">{user.name}</h4><UserStatusBadge status={user.status} /></div>
                            <p className="mt-0.5 text-sm font-medium text-[#00E5FF]">{user.role}</p>
                            <p className="text-sm text-[#8892A4]">{user.startup}</p>
                        </div>
                    </div>
                </div>
                <div className="border-b border-white/[0.06] p-6">
                    <h5 className="mb-3 text-xs font-bold tracking-[0.15em] text-[#8892A4] uppercase">Contact Information</h5>
                    <div className="grid grid-cols-2 gap-3">
                        {[{ icon: Mail, label: 'Email', value: user.email }, { icon: Phone, label: 'Phone', value: user.phone }, { icon: Calendar, label: 'Joined', value: user.joinDate }, { icon: Shield, label: 'Status', value: user.status }].map((info, i) => (
                            <motion.div key={info.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }}
                                className="rounded-xl bg-white/[0.03] p-3 border border-white/[0.04]">
                                <div className="flex items-center gap-1.5"><info.icon className="h-3 w-3 text-[#00E5FF]/50" /><p className="text-[11px] font-medium text-[#8892A4]">{info.label}</p></div>
                                <p className="mt-0.5 truncate text-sm font-medium text-white">{info.value}</p>
                            </motion.div>
                        ))}
                    </div>
                </div>
                <div className="border-b border-white/[0.06] p-6">
                    <h5 className="mb-2 text-xs font-bold tracking-[0.15em] text-[#8892A4] uppercase">Bio</h5>
                    <p className="text-sm leading-relaxed text-white/70">{user.bio}</p>
                </div>
                <div className="border-b border-white/[0.06] p-6">
                    <h5 className="mb-3 text-xs font-bold tracking-[0.15em] text-[#8892A4] uppercase">Skills & Expertise</h5>
                ...
                <div className="border-b border-white/[0.06] p-6">
                    <h5 className="mb-3 text-xs font-bold tracking-[0.15em] text-[#8892A4] uppercase">Activity Stats</h5>
                    <div className="grid grid-cols-4 gap-2">
                        {[{ l: 'Applied', v: user.applied, c: '#00E5FF' }, { l: 'Accepted', v: user.accepted, c: '#00FFC2' }, { l: 'Rate', v: user.applied > 0 ? `${Math.round((user.accepted / user.applied) * 100)}%` : '0%', c: '#7B2FFF' }, { l: 'Active', v: user.applications.filter(a => a.status === 'Pending' || a.status === 'Under Review').length, c: '#FFB800' }].map(stat => (
                            <div key={stat.l} className="rounded-xl bg-white/[0.03] p-3 text-center border border-white/[0.04]">
                                <p className="text-lg font-bold" style={{ color: stat.c }}>{stat.v}</p>
                                <p className="text-[10px] font-medium text-[#8892A4]">{stat.l}</p>
                            </div>
                        ))}
                    </div>
                </div>
                <div className="p-6">
                    <h5 className="mb-3 text-xs font-bold tracking-[0.15em] text-[#8892A4] uppercase">Application History</h5>
                    <div className="flex flex-col gap-2">
                        {user.applications.map((app, i) => (
                            <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.06 }}
                                ...
                                <div className="flex-1"><p className="text-sm font-medium text-white">{app.eventTitle}</p><p className="text-xs text-[#8892A4]">{app.date}</p></div>
                ...
                <div className="border-t border-white/[0.06] p-6">
                    <h5 className="mb-3 text-xs font-bold tracking-[0.15em] text-[#8892A4] uppercase">Social Links</h5>
                    <div className="flex flex-col gap-2">
                        {[{ label: 'LinkedIn', value: user.linkedin }, { label: 'GitHub', value: user.github }].map(link => (
                            <a key={link.label} href={`https://${link.value}`} target="_blank" rel="noopener noreferrer"
                                className="flex items-center gap-2 rounded-xl border border-white/[0.06] p-3 text-sm font-medium text-white hover:bg-white/[0.04] transition-colors">
            ...
            <div className="flex items-center gap-3 border-t border-white/[0.06] p-4">
                ...
                {user.status === 'Suspended' ? (
                    ...
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                        className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-[#FF4757]/30 bg-[#FF4757]/10 px-4 py-2.5 text-sm font-semibold text-[#FF4757]"><Ban className="h-4 w-4" />Suspend</motion.button>
                )}
            </div>
        </motion.div>
    );
                ...
                <div className="flex items-center gap-1 rounded-xl border border-white/[0.08] bg-[#0F1628] p-1">
                    {['All', 'Active', 'Pending', 'Suspended'].map(s => (
                        <button key={s} onClick={() => setStatusFilter(s)}
                            className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-all duration-200 ${statusFilter === s ? 'bg-[#00E5FF] text-[#0A0E1A] shadow-sm shadow-[#00E5FF]/30' : 'text-[#8892A4] hover:bg-white/5 hover:text-white'}`}>{s}</button>
                    ))}
                </div>
            </motion.div>
                    ...
                    <motion.div key={user.id} variants={cardItem}
                        ...
                        <div className="flex items-start gap-4 p-5">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2"><h3 className="truncate text-base font-bold text-white">{user.name}</h3><UserStatusBadge status={user.status} /></div>
                                <p className="text-sm font-medium text-[#00E5FF]">{user.role}</p>
                                <p className="text-sm text-[#8892A4]">{user.startup}</p>
                                <p className="mt-0.5 truncate text-xs text-[#8892A4]/70">{user.email}</p>
                            </div>
                        </div>
                        <div className="mx-5 mb-4 grid grid-cols-3 gap-2">
                            {[{ l: 'Applied', v: user.applied, c: '#00E5FF' }, { l: 'Accepted', v: user.accepted, c: '#00FFC2' }, { l: 'Rate', v: user.applied > 0 ? `${Math.round((user.accepted / user.applied) * 100)}%` : '0%', c: '#7B2FFF' }].map(s => (
                                <div key={s.l} className="rounded-xl bg-white/[0.03] p-2.5 text-center border border-white/[0.04]"><p className="text-sm font-bold" style={{ color: s.c }}>{s.v}</p><p className="text-[10px] text-[#8892A4]">{s.l}</p></div>
                            ))}
                        </div>
                        <div className="mx-5 mb-4 flex flex-wrap gap-1.5">
                            ...
                            {user.skills.length > 3 && <span className="rounded-full bg-white/[0.04] px-2 py-0.5 text-[10px] text-[#8892A4]">+{user.skills.length - 3}</span>}
                        </div>
