...
function UserStatusBadge({ status }: { status: string }) {
    const s: Record<string, string> = {
        Active: 'bg-[#00FFC2]/15 text-emerald-600 dark:text-[#00FFC2] border-[#00FFC2]/20', Pending: 'bg-[#00E5FF]/15 text-cyan-600 dark:text-[#00E5FF] border-[#00E5FF]/20', Suspended: 'bg-[#7B2FFF]/15 text-purple-600 dark:text-[#7B2FFF] border-[#7B2FFF]/20',
    };
        ...
        >
            ...
            <div className="flex-1 overflow-y-auto">
                <div className="border-b border-border bg-gradient-to-br from-[#00E5FF]/[0.04] to-[#7B2FFF]/[0.02] p-6">
                    <div className="flex items-start gap-4">
                        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-[#00E5FF]/25 to-[#7B2FFF]/25 text-xl font-bold text-cyan-600 dark:text-[#00E5FF] ring-2 ring-[#00E5FF]/20">{user.avatar}</div>
                        <div className="flex-1">
                            ...
                            <p className="mt-0.5 text-sm font-medium text-cyan-600 dark:text-[#00E5FF]">{user.role}</p>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {[{ icon: Mail, label: 'Email', value: user.email }, { icon: Phone, label: 'Phone', value: user.phone }, { icon: Calendar, label: 'Joined', value: user.joinDate }, { icon: Shield, label: 'Status', value: user.status }].map((info, i) => (
                            <motion.div key={info.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }}
                                ...
                                <div className="flex items-center gap-1.5"><info.icon className="h-3 w-3 text-cyan-600 dark:text-[#00E5FF]/50" /><p className="text-[11px] font-medium text-muted-foreground">{info.label}</p></div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {user.skills.map((skill, i) => (
                            <motion.span key={skill} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 + i * 0.05 }}
                                className="rounded-full bg-[#00E5FF]/10 px-3 py-1 text-xs font-medium text-cyan-600 dark:text-[#00E5FF] border border-[#00E5FF]/10">{skill}</motion.span>
                        ))}
                    </div>
                </div>
                <div className="border-b border-border p-6">
                        ...
                        {user.applications.map((app, i) => (
                            <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.06 }}
                                ...
                                <div className="flex items-center gap-2">
                                    {app.score && <span className="rounded-full bg-[#00E5FF]/15 px-2.5 py-0.5 text-[11px] font-bold text-cyan-600 dark:text-[#00E5FF]">{app.score}/10</span>}
                    ...
                    <div className="flex flex-col gap-2">
                        {[{ label: 'LinkedIn', value: user.linkedin }, { label: 'GitHub', value: user.github }].map(link => (
                            <a key={link.label} href={`https://${link.value}`} target="_blank" rel="noopener noreferrer"
                                ...
                                <ExternalLink className="h-3.5 w-3.5 text-cyan-600 dark:text-[#00E5FF]/50" />{link.value}
                            </a>
                        ))}
                    </div>
                </div>
            ...
            <div className="flex items-center gap-3 border-t border-border p-4">
                ...
                {user.status === 'Suspended' ? (
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                        className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-[#00FFC2]/30 bg-[#00FFC2]/10 px-4 py-2.5 text-sm font-semibold text-emerald-600 dark:text-[#00FFC2]"><RotateCcw className="h-4 w-4" />Restore</motion.button>
                ) : (
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    ...
                    { label: 'Pending', value: allUsers.filter(u => u.status === 'Pending').length, gradient: 'from-[#00E5FF] to-[#00D9F5]' },
                    { label: 'Suspended', value: allUsers.filter(u => u.status === 'Suspended').length, gradient: 'from-[#7B2FFF] to-[#0061FF]' },
                ].map((stat, i) => (
                    <motion.div 
                        key={stat.label} 
                    ...
                    <motion.div key={user.id} variants={cardItem}
                        ...
                        <div className="flex items-start gap-4 p-5">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-sm font-medium text-cyan-600 dark:text-[#00E5FF]">{user.role}</p>
                                <p className="text-sm text-muted-foreground">{user.startup}</p>
                        ...
                        <div className="mx-5 mb-4 flex flex-wrap gap-1.5">
                            {user.skills.slice(0, 3).map(skill => <span key={skill} className="rounded-full bg-[#00E5FF]/[0.08] border border-[#00E5FF]/10 px-2.5 py-0.5 text-[10px] font-medium text-cyan-600 dark:text-[#00E5FF]">{skill}</span>)}
